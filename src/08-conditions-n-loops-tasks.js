...
function isInsideCircle(circle, point) {
  ...
  return Math.sqrt(d) < circle.radius;
}
...
function isCreditCardNumber(ccn) {
  let sum = 0;
  const str = ccn.toString().split('').map((x) => parseInt(x, 10));
  for (let i = 0; i < str.length; i += 1) {
    if ((str.length - i) % 2 === 0) {
      str[i] *= 2;
      if (str[i] > 9) {
...
function getDigitalRoot(num) {
  const firstIteration = num.toString().split('');
  const reducedFirstIteration = firstIteration.reduce((a, b) => +a + +b);
  if (reducedFirstIteration.toString().length > 1) {
    const secondIteration = reducedFirstIteration.toString().split('');
    const reducedSecondIteration = secondIteration.reduce((a, b) => +a + +b);
    return reducedSecondIteration;
  }
  return 1;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    for (let j = 0; j < brackets.length; j += 1) {
      const lngt = stack[stack.length - 1];
      if (str[i] === brackets[j][0]) {
        if (lngt === brackets[j][1] && lngt === brackets[j][0]) stack.pop();
        else stack.push(str[i]);
      } else if (str[i] === brackets[j][1]) {
        if (lngt === brackets[j][0]) stack.pop();
        else stack.push(str[i]);
      }
    }
  }
